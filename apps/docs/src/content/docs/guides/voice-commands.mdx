...
Both clients post the same payload to `/v1/commands`. Every request carries
`X-Install-ID` to identify the installation and `X-Idempotency-Key` so a retry cannot
...
```kotlin
val response = client.post("/v1/commands") {
    header("X-Install-ID", installId)
    header("X-Idempotency-Key", UUID.randomUUID().toString())
    contentType(ContentType.Application.Json)
    setBody(
        CommandRequest(
            audioBase64 = buffer.toBase64(),
            durationMs = buffer.durationMs,
            language = "am-ET",
            screenContext = ScreenContext(
                currentPackage = activePackageName,
                viewTreeSummary = reducedTree,
            ),
        )
    )
}
...
const response = await fetch('/v1/commands', {
	...
	body: JSON.stringify({
		...
		language: 'am-ET',
		screen_context: {
			current_package: activePackageName,
			view_tree_summary: reducedTree,
		},
	}),
});
```
...
`duration_ms` must fall between 400 and 15000. Anything shorter is discarded before it
reaches the transcriber, and anything longer is rejected outright.
